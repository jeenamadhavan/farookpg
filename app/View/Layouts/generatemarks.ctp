    ...
    <div class="row text-center">
        <div class="col-md-4 col-md-offset-4">
            <div class="panel panel-default">
                <div class="panel-heading"> <strong class="">PG Admission</strong>
...
// doc.line(5, 15, 293, 15); // horizontal line
count = 0;
new_page=true;

if(new_page==true)
    {
doc.rect(5, 5, 288, 200);
        new_page = false;
        if(count!=0)
      doc.addPage();
doc.text(6, 12, 'Apllication No');
doc.line(29, 5, 29, 205); // vertical line
doc.text(50, 12, 'Name');
doc.line(75, 5, 75, 205); // vertical line
doc.text(77, 12, 'Mobile No');
doc.line(93, 5, 93, 205); // vertical line
doc.text(95, 12, 'Qualifying Exam');
doc.line(120, 5, 120, 205); // vertical line
doc.text(124, 12, 'Appplied for');
doc.line(148, 5, 148, 205); // vertical line
doc.text(150, 12, 'Community');
doc.line(180, 5, 180, 205); // vertical line
doc.text(181, 12, 'NCC');
doc.line(189, 5, 189, 205); // vertical line
doc.text(190, 12, 'NSS');
doc.line(198, 5, 198, 205); // vertical line
doc.text(199, 12, 'University studied');
doc.line(227, 5, 227, 205); // vertical line
doc.text(229, 12, 'index');
doc.line(244, 5, 244, 205); // vertical line
doc.text(245, 12, 'office use');
doc.setFontType("normal");
doc.setFontSize(8);

var i=0;
<?php foreach($All_result as $All_result) {?>
 doc.line(5, 15+(i*8), 293, 15+(i*8)); // horizontal line
    doc.text(8, 20+(i*8), '<?php echo $All_result['Choice']['application_no']; ?>');
    var Name="<?php echo $All_result['User']['frkUserName']; ?>";
    Name = Name.substring(0,23);
    doc.text(30, 20+(i*8), Name);
    doc.text(76, 20+(i*8), '<?php echo $All_result['User']['frkUserMobile']; ?>');
    doc.text(95, 20+(i*8), '<?php echo $All_result['Degree']['name']; ?>');
     var Appplied4="<?php echo $All_result['Course']['frkCourseName']; ?>";
    Appplied4 = Appplied4.substring(0,17);
    doc.text(124, 20+(i*8), Appplied4);
    var community="<?php echo $All_result['Community']['name']; ?>";
    if(community!='SC' && community!='ST' && community!='MUSLIM / MAPPILA' )
    {
        community="General";
    }
    if(community=='MUSLIM / MAPPILA')
     community="Muslm";   
    doc.text(150, 20+(i*8), community);
     var NccA="<?php echo $All_result['Reservation']['NCC_Certificate_A']; ?>";
    var NccB="<?php echo $All_result['Reservation']['NCC_Certificate_B']; ?>";
    var NccC="<?php echo $All_result['Reservation']['NCC_Certificate_C']; ?>";
    if(NccA ==1)
        var Ncc='A';
    else if(NccB ==1)
        var Ncc='B';
        ...
        var Ncc="NA";
    doc.text(181, 20+(i*8), Ncc);
   var nss="<?php echo $All_result['Reservation']['frkNcc/Nss']; ?>";
   if(nss==0)
    nss="NA";
else
    nss="Yes";
    doc.text(190, 20+(i*8), nss);
    var university="<?php echo $All_result['Universitie']['name']; ?>";
    university = university.substring(0,20);
    doc.text(199, 20+(i*8), university);
    var index="<?php echo $All_result['Index']['index']; ?>";
    index=index.substring(0,8);
    doc.text(229, 20+(i*8), index);
    doc.text(245, 20+(i*8), '');
    i++;
    count = count + 1;
    if(count%23==0)
...
doc.save('<?php echo $All_result['Course']['frkCourseName']; ?>.pdf');
