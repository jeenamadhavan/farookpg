...
class PagesController extends AppController {
    ...
    public function confirmationlink($randomstring = NULL, $id = NULL) {
        ...
        if (isset($result['User']['frkRandomString']) && $result['User']['frkRandomString'] != " ") {
            $sql = "UPDATE `users` SET `frkUserStatus` = '1', `frkRandomString` = 'verified' WHERE `users`.`frkUserID` = " . $id . "; ";
...
//$appenddata['PrimaryRegister']['carrer-ambition'] = $applicant['Applicant']['frkApplicantAmbition'];
            ...
            if ($this->request->is('post')) {
                ...
                // to pass the choices to ranking
            ...
            if(!empty($this->request->data['SecondaryRegister'])){
                if (!empty($reservations)) {
                    ...
                    if (!$this->Reservation->updateAll($ReservationTableData, $cnd3)) {
                        ...
                        if($Indexes->indexing($this->Session->read('User.userid')) && $Indexes->ranking($choice_arr)) {
...
//pr($choice_result[0]);exit;
  ...
  public function generatepdfapplication1($choice_select)
    {
//        echo $choice_select; exit;
        $this->layout = 'generatepdfapplication';
    // $this->autoRender= false;
